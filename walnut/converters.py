...
from walnut.models import Category, Metalist, GeneCollections, RunInfo, CategoryMeta, MetaDimred, SingleDimred, SpatialInfo, LensImageInfo, GraphCluster, GraphClusterDetail
...
class IOLens(IOConverter[LensImageInfo]):
    ...
    def to_str(content: LensImageInfo) -> str:
        return content.json()
class IOGraphCluster(IOConverter[GraphCluster]):
    @staticmethod
    def from_str(s: str) -> GraphCluster:
        return GraphCluster.parse_obj(json.loads(s))

    @staticmethod
    def to_str(content: GraphCluster) -> str:
        return content.json()
